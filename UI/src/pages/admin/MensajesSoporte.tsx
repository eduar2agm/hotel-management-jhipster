...
import { MessageSquare, Check, Plus, Search } from 'lucide-react';
...
export const AdminMensajesSoporte = () => {
    ...
    const handleReply = (msg: MensajeSoporteDTO) => {
        setCurrentItem({
            fechaMensaje: new Date().toISOString(),
            remitente: Remitente.ADMINISTRATIVO,
            reserva: msg.reserva, // Link to same reservation
            destinatarioId: msg.userId, // Reply to the sender
    ...
    const handleMarkAsRead = async (msg: MensajeSoporteDTO) => {
        try {
            await MensajeSoporteService.updateMensaje(msg.id!, { ...msg, leido: true });
    ...
    return (
        <DashboardLayout title="Centro de Soporte" role="Administrador">
            <Card>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                filteredMensajes.map((msg) => {
                                    ...
                                    const isFromClient = msg.remitente === Remitente.CLIENTE;
                                    ...
                                    return (
                                        <TableRow key={msg.id} className={!msg.leido && !isSent ? 'bg-muted/50 font-medium' : ''}>
                                            ...
                                            <TableCell className="max-w-[400px] truncate" title={msg.mensaje}>
                                                {msg.mensaje}
                                            </TableCell>
                                            <TableCell>
                                                {msg.leido ? (
                                                    ...
                                                    <Badge variant="destructive">No leído</Badge>
                                                )}
                                            </TableCell>
                                            <TableCell className="text-right space-x-2">
                                                {!msg.leido && !isSent && isFromClient && (
                                                    <Button variant="ghost" size="icon" title="Marcar como leído" onClick={() => handleMarkAsRead(msg)}>
