        ...
        <>
            <Card className="bg-card border-0 shadow-xl overflow-hidden rounded-2xl">
                <CardHeader className="border-b border-border bg-card p-6 flex flex-row items-center justify-between">
                    <div>
                        <CardTitle className="text-2xl font-bold text-foreground">Categorías de Habitación</CardTitle>
                        <p className="text-muted-foreground text-sm mt-1">Defina los tipos de habitaciones y sus precios base.</p>
                    </div>
                    <Button
                    ...
                    <Table>
                        <TableHeader className="bg-muted/50">
                            <TableRow className="border-border">
                                <TableHead className="font-bold text-muted-foreground py-5 pl-8">NOMBRE</TableHead>
                                <TableHead className="font-bold text-muted-foreground py-5">DESCRIPCIÓN</TableHead>
                                <TableHead className="font-bold text-muted-foreground py-5">PRECIO BASE</TableHead>
                        ...
                        <TableBody>
                            {isLoading ? (
                                ...
                                <TableRow><TableCell colSpan={4} className="h-32 text-center text-muted-foreground">No hay categorías registradas.</TableCell></TableRow>
                            ) : (
                                categorias.map(cat => (
                                    <TableRow key={cat.id} className="hover:bg-muted/50 transition-colors border-b border-border">
                                        <TableCell className="py-5 pl-8">
                                            <div className="flex items-center gap-4">
                                                <div className={`w-3 h-12 rounded-full shadow-sm ${cat.nombre === 'SUITE' ? 'bg-yellow-400' :
                                                    cat.nombre === 'DOBLE' ? 'bg-blue-400' : 'bg-muted'
                                                    }`}></div>
                                                <span className="font-bold text-foreground text-lg">{cat.nombre}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="py-5 text-muted-foreground text-base">{cat.descripcion}</TableCell>
                                        <TableCell className="py-5">
                                            <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 py-1.5 px-4 rounded-full font-bold text-sm shadow-sm border border-green-200 dark:border-green-800">
                                                ${cat.precioBase?.toLocaleString()}
                                            </span>
                                        </TableCell>
                                        <TableCell className="py-5 text-right pr-8">
                                            <div className="flex justify-end gap-2">
                                                <Button variant="ghost" size="icon" className="h-10 w-10 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-full" onClick={() => { setCurrentCat(cat); setIsDialogOpen(true); }}>
                                                    <Pencil className="h-5 w-5" />
                                                </Button>
                                                <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 dark:hover:text-red-400 rounded-full" onClick={() => cat.id && handleDelete(cat.id)}>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold text-foreground">{currentCat.id ? 'Editar' : 'Crear'} Categoría</DialogTitle>
                        <DialogDescription>
                    ...
                    <form onSubmit={handleSave} className="grid gap-6 py-4">
                        <div className="grid gap-2">
                            <Label className="text-sm font-semibold text-foreground">Tipo de Categoría</Label>
                            <Select
