...
export const ClientReservationCard = ({
    ...
    return (
        <div
            className={`bg-card group transition-all duration-300 border overflow-hidden rounded-xl
                ${isActive ? 'border-yellow-500 shadow-xl ring-2 ring-yellow-500 ring-offset-2' : 'border-border hover:shadow-lg'}
            `}
        >
            {/* Header */}
            <div className="bg-muted/20 px-6 py-4 flex flex-wrap justify-between items-center border-b border-border">
            ...
            <div className="p-6">
                {/* Dates & Location */}
                <div className="flex flex-col sm:flex-row gap-8 mb-8 pb-8 border-b border-dashed border-border">
                    <div className="flex-1 grid grid-cols-2 gap-6">
                        <div>
                            <span className="text-xs text-muted-foreground font-bold uppercase block mb-2 tracking-wider">Check-in</span>
                            <span className="text-xl font-serif font-bold text-foreground flex items-center gap-2">
                                <div className="w-1 h-8 bg-green-500 rounded-full"></div>
                                {formatDate(reserva.fechaInicio)}
                            </span>
                        </div>
                        <div>
                            <span className="text-xs text-muted-foreground font-bold uppercase block mb-2 tracking-wider">Check-out</span>
                            <span className="text-xl font-serif font-bold text-foreground flex items-center gap-2">
                    ...
                    <div className="sm:text-right">
                        <span className="text-xs text-muted-foreground font-bold uppercase block mb-2 tracking-wider">Total Estancia</span>
                ...
                <div className="space-y-4">
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3">Habitaciones Reservadas ({details.length})</h4>
                    {details.map((detalle, idx) => (
                        <div key={detalle.id || idx} className="flex items-center gap-4 p-3 bg-muted/30 rounded-lg border border-border hover:border-yellow-200 transition-colors">
                            {/* Image Thumbnail */}
                            ...
                            <div className="flex-grow">
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <span className="text-xs text-muted-foreground font-medium bg-card px-2 py-0.5 rounded border border-border inline-block mt-1">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className="text-[10px] text-muted-foreground uppercase tracking-wide">/ noche</span>
                                    </div>
                                </div>
                            </div>
                        </div>
